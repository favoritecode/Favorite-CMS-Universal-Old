<?php
$GLOBALS['fm_canonical_header_rendered'] = true;
require_once __DIR__ . '/functions.php';

try {
    $siteTitle = $siteTitle ?? (class_exists(\FavoriteCMS\Models\Setting::class) ? \FavoriteCMS\Models\Setting::get('general', 'site_name', 'Favorite CMS') : 'Favorite CMS');
    $siteTagline = $siteTagline ?? (class_exists(\FavoriteCMS\Models\Setting::class) ? \FavoriteCMS\Models\Setting::get('general', 'site_description', 'Entertainment Streaming Experience') : 'Entertainment Streaming Experience');
    $metaTitle = $metaTitle ?? $siteTitle;
    $metaDesc = $metaDescription ?? (class_exists(\FavoriteCMS\Models\Setting::class) ? \FavoriteCMS\Models\Setting::get('seo', 'meta_description', '') : '');
} catch (\Throwable) {
    $siteTitle = $siteTitle ?? 'Favorite CMS';
    $siteTagline = $siteTagline ?? 'Entertainment Streaming Experience';
    $metaTitle = $metaTitle ?? $siteTitle;
...
if (class_exists(\FavoriteCMS\Multimedia\Theme\ThemeManager::class)) {
    try {
        ...
        $tokensHtml = $themeManager->renderHeadTokens();
    } catch (\Throwable) {}
}

$headerStyle = $themeConfig ? (string)$themeConfig->get('header', 'style', 'glass') : 'glass';
$isSticky = $themeConfig ? (bool)$themeConfig->get('header', 'sticky', true) : true;
$brandTitle = $themeConfig ? (string)$themeConfig->get('branding', 'brand_title', $siteTitle) : $siteTitle;
$logoUrl = $themeConfig ? (string)$themeConfig->get('branding', 'logo_url', '') : '';

$isPluginActive = favorite_multimedia_theme_is_plugin_active();
$isCompatible = favorite_multimedia_theme_is_compatible();
$isLoggedIn = favorite_multimedia_theme_is_logged_in();
$currentUser = $isLoggedIn ? favorite_multimedia_theme_current_user() : null;
$userDisplayName = $isLoggedIn ? favorite_multimedia_theme_user_display_name($currentUser) : '';
$userInitials = $isLoggedIn ? favorite_multimedia_theme_user_initials($currentUser) : '';
$canAccessAdmin = $isLoggedIn ? favorite_multimedia_theme_can_access_admin($currentUser) : false;
$canCreatePosts = $isLoggedIn ? favorite_multimedia_theme_can_create_posts($currentUser) : false;
...
$digitalAvailable = class_exists(\FavoriteCMS\Multimedia\Integrations\FavoriteDigitalAdapter::class) && \FavoriteCMS\Multimedia\Integrations\FavoriteDigitalAdapter::isAvailable();
...
<!-- Theme Persistent Top Navigation Bar -->
<header class="fm-header fm-header-<?php echo htmlspecialchars($headerStyle, ENT_QUOTES, 'UTF-8'); ?> <?php echo $isSticky ? 'fm-header-sticky' : ''; ?>" style="z-index: 1000;">
    <div class="fm-header-inner" style="display: flex; align-items: center; justify-content: space-between; width: 100%; max-width: var(--fm-layout-max-width, 1440px); margin: 0 auto; padding: 0 24px; height: 68px; box-sizing: border-box;">
        <div style="display: flex; align-items: center; gap: 32px;">
            <a href="/" class="fm-brand-link" aria-label="<?php echo htmlspecialchars($brandTitle, ENT_QUOTES, 'UTF-8'); ?>">
            ...
            <nav class="fm-desktop-nav" aria-label="Primary Navigation" style="display: flex; gap: 20px; font-weight: 600; font-size: 0.95rem;">
                <a href="/" class="fm-nav-link <?php echo $navCurrent === 'home' ? 'active' : ''; ?>" <?php echo $navCurrent === 'home' ? 'aria-current="page"' : ''; ?>>Home</a>
        ...
        <div class="fm-header-actions" style="display: flex; align-items: center; gap: 16px;">
            <!-- Expanding Search Pill beside Profile -->
            ...
            <?php else: ?>
                <a href="/admin/login" class="fm-auth-btn fm-btn-signin" style="font-size: 0.85rem; font-weight: 600; background: var(--fm-color-primary, #e50914); color: #fff; padding: 6px 16px; border-radius: 20px; text-decoration: none;">Sign In</a>
            <?php endif; ?>
        </div>
    </div>
</header>
